...
const BookInfoPanel = ({ book, isOpen, onClose, progress, currentChapter, currentPage, totalPages }) => {
  ...
  useEffect(() => {
    if (isOpen && book) {
      fetchBookDetails();
      setShowCharacters(false); // 패널이 열릴 때마다 인물 숨김 상태로 초기화
    }
  }, [isOpen, book]);
          ...
          <>
            ...
            {bookDetails && (
              <>
                ...
                {/* 마지막으로 읽은 시점 */}
                <div style={sectionStyle}>
                  <div style={labelStyle}>마지막으로 읽은 시점</div>
                  <div style={valueStyle}>
                    {currentChapter && (
